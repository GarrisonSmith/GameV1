...
namespace Fantasy.Content.Logic.graphics
{
    class Animation
    {
        ...
        public void DrawAnimation(Point position, Texture2D texture, Color color, Vector2 _stretch, SpriteBatch _spriteBatch)
        {
            switch (animationState)
            {
                case AnimationState.cycling:
                    if (Game1._gameTime.TotalGameTime.TotalMilliseconds - lastFrameGameTime >= frameDuration)
                    {
                        lastFrameGameTime = Game1._gameTime.TotalGameTime.TotalMilliseconds;
